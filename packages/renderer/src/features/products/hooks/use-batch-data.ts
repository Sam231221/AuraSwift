...
export const useBatchData = ({ businessId, productId }: UseBatchDataProps) => {
  ...
  const [expirySettings, setExpirySettings] = useState<ExpirySettings | null>(null);
  ...
  const loadBatches = useCallback(async () => {
    ...
    try {
      ...
      const response = await window.batchAPI.getByBusiness(businessId, productId);
  ...
  const updateBatch = useCallback(
    async (
      ...
      try {
        ...
        if (response.success && response.batch) {
          setBatches((prev) =>
            prev.map((batch) => (batch.id === batchId ? response.batch! : batch))
          );
        }
  ...
  const deleteBatch = useCallback(async (batchId: string): Promise<BatchResponse> => {
      try {
        if (!window.batchAPI) {
          return {
            success: false,

...
import type { ExpiryAlert } from "@/features/inventory/types/batch.types";
import { formatExpiryDate } from "@/features/inventory/utils/expiry-calculations";
...
const ExpiryAlertCenter: React.FC<ExpiryAlertCenterProps> = ({
  ...
  const getAlertVariant = (type: string): "default" | "destructive" => {
    return type === "EXPIRED" || type === "CRITICAL"
      ? "destructive"
      : "default";
  };
            ...
            <AlertDescription className="mt-2">
              ...
              <div className="mt-2 flex items-center space-x-4 text-xs text-gray-600">
                <span>
                  ...
                  {formatExpiryDate(alert.batch.expiryDate, false)}
                </span>
                <span>
                ...
                {alert.batch.costPrice && (
                  <span>
                    <strong>Value:</strong> £
                    {(
                      alert.batch.costPrice * alert.batch.currentQuantity
                    ).toFixed(2)}
                  </span>
                )}
              </div>
    ...
    <div className="p-6 space-y-6">
      ...
      {criticalAlerts.length > 0 && (
        <Card className="border-red-200">
          ...
          <CardContent>{criticalAlerts.map(renderAlertCard)}</CardContent>
        </Card>
      )}
      ...
      {warningAlerts.length > 0 && (
        <Card className="border-orange-200">
          ...
          <CardContent>{warningAlerts.map(renderAlertCard)}</CardContent>
        </Card>
      )}
      ...
      {infoAlerts.length > 0 && (
        <Card className="border-blue-200">
          ...
          <CardContent>{infoAlerts.map(renderAlertCard)}</CardContent>
        </Card>
      )}

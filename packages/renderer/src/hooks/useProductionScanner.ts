  ...
  const defaultValidateBarcode = useCallback(
    (barcode: string): boolean => {
      ...
      if (!validLengths.includes(cleanBarcode.length)) {
        console.warn(`Uncommon barcode length: ${cleanBarcode.length}`);
      ...
      if (!validCharPattern.test(cleanBarcode)) {
        console.warn("Barcode contains invalid characters");
        return false;
      }
  ...
  const processScannedBarcode = useCallback(
    async (barcode: string) => {
      if (isProcessingRef.current) {
        console.warn("Already processing a scan, ignoring duplicate");
        return;
      }
      ...
      try {
        ...
        if (scanResult) {
          ...
          console.warn("❌ Barcode scan failed - product not found:", barcode);
          setScannerStatus("error");
          setTimeout(() => setScannerStatus("ready"), 1500);
        }
      } catch (error) {
